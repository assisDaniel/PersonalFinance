...
class CategoriaAdmin(ModelAdmin):
    ...
    list_display = ('id', 'tipo_colored', 'nome')
    list_display_links = ('id', 'tipo_colored')
    ordering = ('id', 'tipo')
    actions = ('desativarCategoria',)
    ...
    @admin.action(description='1 - Desativar categoria(s) selecionada(s)')
    def desativarCategoria(self, request, queryset):
        updated = queryset.update(ativo=False)
        self.message_user(request, f"{updated} categoria(s) desativada(s).")
